...
class VoucherController extends Controller
{
    public function applyVoucher(Request $request)
    {
        ...
        // Kiểm tra ngày hiện tại và so sánh với ngày bắt đầu/kết thúc của voucher
        $currentDate = Carbon::now();
        Log::info('Current date', ['current_date' => $currentDate->toDateTimeString()]);

        if ($voucher->start_date && Carbon::parse($voucher->start_date)->gt($currentDate)) {
        ...
        if ($voucher->end_date && Carbon::parse($voucher->end_date)->lt($currentDate)) {
...
Log::info('Cart total calculated', ['cartTotal' => $cartTotal]);
        ...
        if ($voucher->type === 'percentage') {
            // Ép kiểu discount về float và chia cho 100
